...
export async function getRoute(
  ...
  try {
    ...
    const steps = route.legs[0].steps.map((step) => ({
      instruction:
        step.instruction || generateInstructionFromManeuver(step.maneuver),
...
function generateFallbackRoute(
  ...
  const steps = [
    ...
    },
    {
      ...
      coordinates: [
        start.lng + (end.lng - start.lng) * 0.5,
        start.lat + (end.lat - start.lat) * 0.5,
      ] as [number, number],
    },
    {
...
function generateInstructionFromManeuver(maneuver: {
  type: string;
  modifier?: string;
}): string {
...
function routePassesThroughFloodZones(
  ...
  const dangerousFloodZones = floodZones.filter(
    (zone) => zone.severity === "Critical" || zone.severity === "High"
  );
...
function distanceToLineSegment(
  ...
  const t =
    ((x1 - x3) * (x4 - x3) + (y1 - y3) * (y4 - y3)) / (lineLength * lineLength);
...
export async function findSafeRoute(
  ...
  try {
    ...
    const passesThroughFloodZones = routePassesThroughFloodZones(
      directRoute.coordinates,
      floodZones
    );
    const passesThroughBlockedRoads = routePassesThroughBlockedRoads(
      directRoute.coordinates,
      blockedRoads
    );
    ...
    console.log(
      "Direct route passes through flood zones or blocked roads, finding alternatives"
    );
...
function generateWaypoints(
  ...
  for (const zone of floodZones) {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      for (const checkZone of floodZones) {
        if (
          isPointInPolygon([waypointLng, waypointLat], checkZone.coordinates)
        ) {
  ...
  for (const road of blockedRoads) {
    ...
    waypoints.push([
      midLat + normPerpDx * detourDistance,
      midLng + normPerpDy * detourDistance,
    ]);
    waypoints.push([
      midLat - normPerpDx * detourDistance,
      midLng - normPerpDy * detourDistance,
