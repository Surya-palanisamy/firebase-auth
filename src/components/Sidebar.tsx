...
export default function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const general = [{ label: "Emergency Help", to: "/emergency-help", icon: PhoneCall }];
    ...
    <Box sx={{ width: 260, display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      >
        ...
        <MenuItem onClick={() => handleMenuSelect("/logout")}>
          <LogOut size={18} style={{ marginRight: 12, color: theme.palette.error.main }} />
          <Typography sx={{ fontSize: 13, color: theme.palette.error.main, fontWeight: 600 }}>
            Log out
          </Typography>
        </MenuItem>
      </Menu>
            ...
            <ListItemButton
              ...
              sx={{
                ...
                ...(active && {
                  ...
                  boxShadow: theme.palette.mode === "dark" ? "0 10px 30px rgba(0,0,0,0.7)" : "0 10px 30px rgba(15,23,42,0.18)",
                }),
                "&:hover": { backgroundColor: active ? "transparent" : theme.palette.action.hover },
              }}
            >
        ...
        <Box onClick={handleProfileMenuOpen} sx={{ width: "100%", height: 56, p: 1, display: "flex", gap: 2, cursor: "pointer", alignItems: "center", borderRadius: 2 }}>
          <Avatar
            ...
            sx={{
              ...
              fontWeight: 700,
              fontSize: 14,
              bgcolor: !avatarURL ? (theme.palette.mode === "dark" ? theme.palette.grey[800] : theme.palette.primary.dark) : "transparent",
            }}
          >
            {!avatarURL && initials}
          </Avatar>
          ...
          <Box sx={{ flex: 1, textAlign: "left", minWidth: 0 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{displayName}</Typography>
            <Typography sx={{ fontSize: 13, color: "text.secondary", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user?.email ?? "Unknown"}</Typography>
          </Box>
        </Box>
      </List>
    </Box>
